...
class HomeServices {
  ...
  Future getPopularProduct() async {
    var url = Uri.parse("${baseUrl + api + v1 + popularProduct}");
    var header = {"Content-Type": "application/json; charset=utf-8"};
    var body = jsonEncode({
      'token': 'token1',
    });
    try {
      ...
      var statusCode = response.statusCode;
      try {
        var _hasilPopular = jsonDecode(response.body);
        print('ini popular : ${_hasilPopular}');

        // Check if status code 404 / 200 / 5xx
        switch (statusCode) {
          case 404:
            var error = _hasilPopular['message'];
            String errorMsg =
                error.reduce((value, elment) => value + ',' + ' ' + elment);
            return ApiReturnValue(value: errorMsg, code: statusCode);
          ...
          case 200:
            List<ResponseProductCard> listPopularProduct =
                ResponseProductCard.fromJsonList(_hasilPopular['product']);
            return ApiReturnValue(
              code: statusCode,
              value: listPopularProduct,
            );
